...
const ButtonGradient = ({ handleCLick, link, title, type }: any) => {
  return (
    <div className="NFLIHPTIDQ">
      {type ? (
        <div className="LOTRKKBQYR">
          <Link className="NRDIIJASKM" href={link} onClick={handleCLick}>
            {title}
          </Link>
          <svg
        ...
        <Link href={link} className="NRDIIJASKM">
          {title}
        </Link>
      )}
    </div>
